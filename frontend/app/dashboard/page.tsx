...
export default function DashboardPage() {
  ...
  if (!token || !mobileNumber) {
    return (
      <main>
        <section className="journey-card" aria-labelledby="dashboard-title">
          <div className="brand">BookMyShow</div>
          <h1 id="dashboard-title">Sign in to choose a movie.</h1>
          <p role="alert">
  ...
  return (
    <main>
      <section className="discovery-shell" aria-labelledby="dashboard-title">
        <div className="brand">BookMyShow · Hyderabad</div>
        <h1 id="dashboard-title">Choose the big screen moment.</h1>
        <p>Pick a film to see its available theatres.</p>
        {loading ? <p role="status">Loading movies…</p> : null}
        {error ? <p className="message message-error" role="alert">{error}</p> : null}
        {!loading && !error && movies.length === 0 ? (
          <p className="message message-status" role="status">
            No movies are available right now. Please try again later.
          </p>
        ) : null}
        {!loading && !error && movies.length > 0 ? (
          <MovieGrid
