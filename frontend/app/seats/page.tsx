...
export default function SeatsPage() {
    ...
    return (
      <main>
        <section className="journey-card">
          ...
          <p role="alert">Sign in and select a movie and theatre before choosing seats.</p>
  ...
  return (
    <main>
      <section className="checkout-shell" aria-labelledby="seats-title">
        <p className="eyebrow">
          {journey.selectedMovie.title} · {journey.selectedTheatre.name}
        </p>
        <h1 id="seats-title">Choose exactly three seats.</h1>
        <SeatGrid
          seats={journey.selectedSeatIds}
          totalPrice={journey.totalPrice}
          onToggleSeat={journey.toggleSeat}
        />
        <button
          type="button"
          disabled={journey.selectedSeatIds.length !== 3}
          onClick={() => {
