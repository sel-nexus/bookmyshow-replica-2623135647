...
export function CheckoutPanel({
  ...
  const cardIsValid = /^\d{13,19}$/.test(normalizedCardNumber)
    && normalizedCardNumber
      ...
      .reduce((sum, digit, index) => {
        ...
        const adjustedValue = doubledValue > 9 ? doubledValue - 9 : doubledValue;

        return sum + (index % 2 === 1 ? adjustedValue : value);
      }, 0) % 10 === 0;
  ...
  return (
    <section className="checkout-panel" aria-labelledby="payment-title">
      <p className="eyebrow">Step 2 · Payment</p>
      <h2 id="payment-title">Choose how you will pay</h2>
      <fieldset>
        ...
        <div className="payment-fields">
          <label>
            ...
            <input
              ...
              aria-invalid={cardNumber.length > 0 && !cardIsValid}
              aria-describedby="card-error"
            />
          </label>
          {cardNumber.length > 0 && !cardIsValid ? (
            <p id="card-error" role="alert">Enter a valid 13–19 digit card number.</p>
          ) : null}
          <label>
            Expiry
            <input
      ...
      <button type="button" disabled={!ready || processing} onClick={onPay}>
